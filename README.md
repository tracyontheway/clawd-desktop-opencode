...
A desktop pet that reacts to your [Claude Code](https://docs.anthropic.com/en/docs/claude-code) and [OpenCode](https://opencode.ai/) sessions in real-time. Clawd lives on your screen — thinking when you prompt, typing when tools run, juggling subagents, celebrating when tasks complete, and sleeping when you're away.

> Windows 11 only. Requires Node.js and Claude Code or OpenCode.

## Features

- **Real-time state awareness** — Claude Code and OpenCode hooks drive Clawd's animations automatically
...
> **OpenCode Users**: See [hooks/README.md](hooks/README.md) for the OpenCode event mapping.
...
Clawd now also supports [OpenCode](https://opencode.ai/)! Use the OpenCode plugin for the same real-time animation experience:
...
# Clawd will react to OpenCode events just like with Claude Code!
```

See [hooks/README.md](hooks/README.md) for detailed OpenCode plugin documentation.
...
```
OpenCode triggers plugin event
  → hooks/clawd-opencode.ts (TypeScript plugin)
  → HTTP POST to 127.0.0.1:23333
  → State machine in main.js (event normalization + multi-session tracking)
  → IPC to renderer.js (SVG preload + crossfade swap)
```

Both Claude Code and OpenCode use the same HTTP endpoint, so Clawd works seamlessly with either tool.
...
hooks/
  ...
  clawd-opencode.ts      # OpenCode plugin (TypeScript)
  install-opencode.js    # OpenCode plugin installer
...
**Cause:**
This error occurs when Electron's binary distribution wasn't downloaded properly during `npm install`. This can happen due to:
- Network issues during initial installation
- Corrupted download
- Incomplete npm install process

**Solution:**
```bash
# Remove the corrupted electron module
rm -rf node_modules/electron

# Reinstall electron
npm install electron

# Now try starting Clawd again
npm start
```

If the issue persists, try clearing the entire node_modules and reinstalling:
```bash
